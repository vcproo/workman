...
$worker->onWorkerStart = function () use ($conf,$DataCDB) {
    ...
    Timer::add(60, function () use ($conf,$DataCDB) {
        ...
        // 每天凌晨 2 点执行
        if (date('H') == '02' && date('i') == '00') { // 凌晨 2:00
        ...
        // 每天上午 12 点执行
        if (date('H') == '12' && date('i') == '00') { // 上午 9:00
            $conf->write_log('每天上午12点执行', '每天上午12点执行');
            $DataCDB->addClient('每天上午12点执行', rand(9999, 99999));
             //备份数据库
             $conf->backup_mysql();
             //备份库里的某一张表
             $conf->backup_mysql_test();
            echo "每天上午12点执行" . date('Y-m-d H:i:s') . "\n";
    ...
    // // 每天凌晨 2 点执行
    // Timer::add(60, function () use ($conf,$DataCDB) { // 每小时检查一次时间
        
    //     if (date('H') == '02') { // 凌晨 2:00
    //         $conf->write_log('每天凌晨2点执行','每天凌晨2点执行');
    //         $DataCDB->addClient('每天凌晨2点执行',rand(9999,99999));
    //         //备份数据库
    //         $conf->backup_mysql();
    ...
    //  // 每天上午 9 点执行
    //  Timer::add(60, function () use ($conf, $DataCDB) { // 每小时检查一次时间
    //     if (date('H') == '11') { // 上午 9:00
    //         $conf->write_log('每天上午11点执行', '每天上午11点执行');
    //         $DataCDB->addClient('每天上午11点执行', rand(9999, 99999));
    //          //备份数据库
    //          $conf->backup_mysql();
    //          //备份库里的某一张表
    //          $conf->backup_mysql_test();
    //         echo "每天上午11点执行" . date('Y-m-d H:i:s') . "\n";
    //         // 这里编写上午 9 点要执行的业务逻辑
